...
The supplied pointing-hand image is bundled as `assets/author-icon.png` and the supplied Crazyland logo as `assets/footer-icon.png`. They are attached directly to each message, and the logo is also reused in the top-right of larger embeds. No image URL variables are needed in Railway. A separate top-right logo can optionally be supplied as `assets/thumbnail-icon.png` or `EMBED_THUMBNAIL_ICON_URL`. Bundled assets take priority over the corresponding image URL settings. The bot needs **Embed Links** and **Attach Files** permissions in the donation and admin channels.
