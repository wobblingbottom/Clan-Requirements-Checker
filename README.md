...
The pointing-hand author icon loads from `assets/author-icon.png`; the footer logo loads from `assets/footer-icon.png`; and the wider top-right logo loads from `assets/thumbnail-icon.png`. When present, they are attached directly to each message. Otherwise set `EMBED_AUTHOR_ICON_URL`, `EMBED_FOOTER_ICON_URL`, and `EMBED_THUMBNAIL_ICON_URL` to direct image URLs in Railway Variables. If no separate thumbnail is configured, the footer logo is reused at the top-right. Missing icons are omitted. The bot needs **Embed Links** permission in the donation and admin channels.
